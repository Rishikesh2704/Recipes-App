...
export default function RecipeUi({ recipe }: { recipe: Promise<any> }) {
  ...
  const instructionSteps = Instructions.split("\n").map((arr:string) =>  {if(!arr.includes('step')) return arr.split('\r')[0]}).filter((arr:string) => arr )
  return (
      ...
      <div className="recipeGrid  h-[98vh] relative w-[74vw]">
        ...
        <ol className="Instructions z-1 bg-[rgba(0,0,0,0.4)] backdrop-blur-lg  relative ">
          ...
          {instructionSteps.map((step: string, idx: number) => {
              return (
              <li key={Math.random()}> {idx+1}. {step}</li>)
          })}
        </ol>

        <ol className="Ingredients z-1   bg-[rgba(0,0,0,0.4)] backdrop-blur-lg relative ">
          <h2 className="italic text-xl font-semibold underline underline-offset-3 absolute top-0">
            Ingredients
          </h2>
        ...
        <div className="Resources z-1 bg-[rgba(0,0,0,0.4)] backdrop-blur-lg relative grid grid-cols-2  gap-5">
          ...
          <div className="flex gap-[0.2rem]">
            {Source && (
              <Link href={Source} target="_blank" className="hover:text-gray-700">
                <i className="fa-solid text-white fa-newspaper"></i>&ensp;Source
              </Link>
            )}
            &ensp;
            {Youtube && (
              <Link href={Youtube} target="_blank" className="hover:text-red-700">
                <i className="fa-brands text-red-500 fa-youtube"></i>&ensp;Video
              </Link>
            )}
          </div>
        </div>
